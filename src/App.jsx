...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .name-btn { width: 100%; background: #f9f6f1; border: 1.5px solid #e0d9ce; border-radius: 10px; padding: 12px 16px; text-align: left; font-family: 'DM Sans', sans-serif; font-size: 0.95rem; color: #2a1f0f; cursor: pointer; transition: border-color 0.15s, background 0.15s; }
  .name-btn:hover { border-color: #8c7355; background: #faf7f2; }
    ...
    <div className="screen">
      <div className="inner">
        ...
        <div style={{ marginBottom:'2rem' }}>
          <span className="badge">主辦人</span>
          <h2 className="serif" style={{ fontSize:'2rem', color:'#2a1f0f', marginTop:'0.6rem' }}>建立房間</h2>
          <p className="muted" style={{ marginTop:'0.4rem' }}>輸入所有參與者的名字</p>
        </div>
        <div className="card">
    ...
    <div className="screen center">
      <div className="inner">
        ...
        <div style={{ textAlign:'center', marginBottom:'2rem' }}>
          ...
          <p className="muted" style={{ marginTop:'0.5rem' }}>輸入主辦人給你的房間代碼</p>
        </div>
        <div className="card">
          <div style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
            <div>
      ...
      <div className="screen center">
        <div className="inner">
          <div className="card fade-up" style={{ textAlign:'center' }}>
            ...
            <p className="muted" style={{ marginBottom:'1rem' }}>答案已送出，等待主辦人開始猜謎</p>
        ...
        <div className="card" style={{ textAlign:'center' }}>
          {!started ? (
            <>
              ...
              <p className="muted" style={{ marginTop:'0.5rem' }}>主辦人將在所有人填完後開始</p>
            </>
          ) : (
            <>
              <div style={{ fontSize:'2.5rem', marginBottom:'1rem' }}>🎉</div>
              <p className="serif" style={{ fontSize:'1.5rem', color:'#2a1f0f' }}>猜謎已開始！</p>
              <p className="muted" style={{ marginTop:'0.5rem' }}>請看主辦人的螢幕一起猜</p>
            </>
          )}
        </div>
      </div>
...
export default function App() {
  ...
  const [hostStep, setHostStep] = useState('setup')
  const [guestStep, setGuestStep] = useState('draw')

  const reset = () => { setMode(null); setRoomCode(null); setMyName(null); setRoom(null); setHostStep('setup'); setGuestStep('draw') }
  ...
  return (
    <>
      ...
      {mode === 'host' && hostStep === 'dashboard' && (
        <HostDashboard code={roomCode} initialRoom={room} onReveal={r => { setRoom(r); setHostStep('reveal') }} />
      )}
